...
ChangeContrast(double factor)
{
  ...
  for(int j = 0; j < height; j++)
  {
    ...
    {
      R2Pixel pixel = Pixel(i, j);
      pixel.SetRed((1-factor)*0.5 + (factor*pixel.Red()));
      pixel.SetGreen((1-factor)*0.5 + (factor*pixel.Green()));
      pixel.SetBlue((1-factor)*0.5 + (factor*pixel.Blue()));
      pixel.Clamp();
      SetPixel(i, j, pixel);
    }
  }
...
Blur(double sigma)
{
    ...
    // The function convolutes the blur filter

  int size = (ceil(3*sigma)*2)+1;
  double **filter = new double*[size];
  double sum = 0.0;
  double sigma_2 = pow((double)sigma,2);
  double d = 1/(2*M_PI*sigma_2);
  int center = size/2;
  //generating filter
  for(int i = 0; i<size ; i++)
  {
    ...
    {
      filter[i][j] = exp(-1*(pow((double)center-i,2)+pow((double)center-j,2))/(2*sigma_2))*d;
      sum+=filter[i][j];
    }
  }
  ...
  R2Image out = R2Image(width,height);
  for(int j = 0; j < height; j++ )
  {
    for(int i = 0; i < width; i++ )
    {
      R2Pixel value = R2null_pixel;
      for(int ii = 0; ii<size; ii++)
      {
        for(int jj = 0; jj<size; jj++)
        {
          if( ii+i-center<0 ||  jj+j-center<0 || ii+i-center>width || jj+j-center>height )
          {
          ;
          }
          else
          {
            R2Pixel pixel = Pixel(ii+i-center,jj+j-center);
            value.SetRed(value.Red()+(pixel.Red()*filter[ii][jj]));
            value.SetGreen(value.Green()+(pixel.Green()*filter[ii][jj]));
            value.SetBlue(value.Blue()+(pixel.Blue()*filter[ii][jj]));
          }
        }
      }
      value.Clamp();
      out.SetPixel(i,j,value);
    }
  }
  *this = out;
}
...
EdgeDetect(void)
{
  ...
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  int size = 3;;
  double filter[3][3] = {{-1,0,1},{-2,0,2},{-1,0,1}};
  int center = size/2;
  R2Image out = R2Image(width,height);
  for(int j = 0; j < height; j++ )
  {
    for(int i = 0; i < width; i++ )
    {
      ...
      for(int ii = 0; ii<size; ii++)
      {
        for(int jj = 0; jj<size; jj++)
        {
          ...
          {
            R2Pixel pixel = Pixel(ii+i-center,jj+j-center);
            //Adding both x and Y component. same filter's transpose is getting used
            value.SetRed(value.Red()+(pixel.Red()*filter[jj][ii])+(pixel.Red()*filter[ii][jj]));
            value.SetBlue(value.Blue()+(pixel.Blue()*filter[jj][ii])+(pixel.Blue()*filter[ii][jj]));
            value.SetGreen(value.Green()+(pixel.Green()*filter[jj][ii])+(pixel.Green()*filter[ii][jj]));
          }
        }
      }
      value.Clamp();
...
Scale(double sx, double sy, int sampling_method)
{
  ...
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  int newheight = ceil(height*sx);
  int newwidth = ceil(width*sy);
  int newi,newj;
  R2Image out = R2Image(newwidth,newheight);
  R2Pixel value = R2null_pixel;
  R2Pixel pixel = R2null_pixel;
  switch(sampling_method)
  {
  //point sampling_method
   case 0:
  for(int j=0;j<newheight;j++)
  {
    for(int i=0;i<newwidth;i++)
    {
      ...
      newi = ceil(i/sx);
      value = R2null_pixel;
      pixel = Pixel(newi,newj);
      value.SetRed(pixel.Red());
      value.SetGreen(pixel.Green());
      value.SetBlue(pixel.Blue());
      value.Clamp();
      out.SetPixel(i,j,value);
    }
  }
  *this = out;
  ...
  //bilinear sampling_method
   ...
   for(int j=0;j<newheight;j++)
   {
     for(int i=0;i<newwidth;i++)
     {
       ...
       value = R2null_pixel;
       pixel = Pixel(newi,newj);
       value.SetRed(pixel.Red()*(1-sx)*(1-sy));
       value.SetGreen(pixel.Green()*(1-sx)*(1-sy));
       value.SetBlue(pixel.Blue()*(1-sx)*(1-sy));
       newi = ceil((i+1)/sx);
       if(newi<width)
       {
         pixel = Pixel(newi,newj);
         value.SetRed(value.Red()+(pixel.Red()*sx*(1-sy)));
         value.SetGreen(value.Green()+(pixel.Green()*sx*(1-sy)));
         value.SetBlue(value.Blue()+(pixel.Blue()*sx*(1-sy)));
       }
       newi = ceil(i/sx);
       newj = ceil((j+1)/sy);
       if(newj<height)
       {
         pixel = Pixel(newi,newj);
         value.SetRed(value.Red()+(pixel.Red()*(1-sx)*sy));
         value.SetGreen(value.Green()+(pixel.Green()*(1-sx)*sy));
         value.SetBlue(value.Blue()+(pixel.Blue()*(1-sx)*sy));
       }
       newj = ceil((j+1)/sy);
       newi = ceil((i+1)/sx);
       if(newj<height && newi<width)
       {
         pixel = Pixel(newi,newj);
         value.SetRed(value.Red()+(pixel.Red()*sx*sy));
         value.SetGreen(value.Green()+(pixel.Green()*sx*sy));
         value.SetBlue(value.Blue()+(pixel.Blue()*sx*sy));
       }
      value.Clamp();
      out.SetPixel(i,j,value);
     }
   ...
   *this = out;
   ...
   //Gaussian sampling_method
   case 2:
   // applying gaussian of the scaling factor
   Blur(sqrt((sx*sx)+(sy*sy)));
   for(int j=0;j<newheight;j++)
   {
     for(int i=0;i<newwidth;i++)
     {
